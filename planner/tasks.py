...
def send_reminders():
    ...
    if not upcoming.exists():
        logger.debug("[Celery Reminders] No upcoming reminders at %s.", window_start.strftime('%H:%M'))
        return
    ...
    for event in upcoming:
        ...
        try:
            if getattr(user_profile, 'email_notifications_enabled', False):
                ...
                if recipient:
                    ...
                    logger.info("Sent reminder email to %s for event %s", recipient, event.pk)
                    sent_any = True
        except Exception as exc:
            logger.exception("Failed to send reminder email for event %s: %s", event.pk, exc)

        # Webhook notification
        try:
            if getattr(user_profile, 'webhook_notifications_enabled', False) and getattr(user_profile, 'webhook_url', None):
                payload = {
                    'event_id': event.pk,
                    'title': event.title,
                ...
                try:
                    resp = requests.post(user_profile.webhook_url, json=payload, timeout=5)
                    resp.raise_for_status()
                    logger.info("Sent webhook to %s for event %s", user_profile.webhook_url, event.pk)
                    sent_any = True
                except Exception as exc:
                    logger.exception("Failed to POST webhook for event %s to %s: %s", event.pk, user_profile.webhook_url, exc)
        except Exception:
        ...
        if sent_any or not (getattr(user_profile, 'email_notifications_enabled', False) or getattr(user_profile, 'webhook_notifications_enabled', False)):
